...
class Read:
    ...
    def __init__(
        ...
        self.quality_array: np.ndarray = np.zeros(
            self.run_read_length
        )  # this will have the correct memory length
    ...
    def apply_errors(self, quality_model: TraditionalQualityModel):
        ...
        for error in self.errors:
            ...
            mutated_sequence = (
                mutated_sequence[: error.location]
                + error.alt
    ...
    def apply_mutations(self, quality_scores: list, rng: Generator):
        ...
        for location in self.mutations:
            ...
            if to_mutate:
                if (
                    type(variant_to_apply) == Insertion
                    or type(variant_to_apply) == SingleNucleotideVariant
                ):
                    reference_length = 1
    ...
    def apply_variants_for_final_output(
        self, quality_model: TraditionalQualityModel, rng: Generator
    ):
        """
    ...
    def finalize_read(
        ...
        """
        Finalizes the read's attributes like quality string and mapping quality.
        This method no longer writes to a file.
        ...
        self.quality_array = qual_model.get_quality_scores(
            err_model.read_length, len(self.reference_segment), rng
        )
        ...
        self.errors, self.padding = err_model.get_sequencing_errors(
            self.padding, self.reference_segment, self.quality_array, rng
        )
        ...
        self.read_quality_string = "".join(
            [chr(int(x) + quality_offset) for x in self.quality_array]
        )
    ...
    def to_fastq_string(self) -> str:
        """
        Returns the FASTQ string representation of the read.
        Assumes finalize_read has already been called.
        """
